    ...
    constructor(params) {
        super({
            ...
            children: [
                ...
                new Div({ "tagName": "p", "position": "static","overflow": "hidden", "height": "80px", "innerHTML": "<b>" + params.appName + "</b>: " + params.appDescription }),
        ...
        if (params.apkLocation) {
            fetch(params.apkLocation,
                ...
                if (res.ok) {
                    console.log("Found: "+params.apkLocation);
        ...
        fetch("/lib/applist.json").then(rawdata => rawdata.json()).then(jsn => {
            ...
            for (let i = 0; i < jsn.data.length; i++) {
                ...
                if (appLocation && app192Logo && !isProbablyArabic(appName) && !isProbablyArabic(appDescription)) {
                    var apkLocation = appLocation.replace("/index.html", "/apk/app-release-signed.apk");
