...
async function getDescription(url)
{
    ...
    try {
        ...
        let dList=resText.match(/<meta .*?name.*?"Description".*?>/g);
        if(dList && dList.length>0)
        {
            console.log("Description="+dList[0].replace(/.*?content.*?"(.*?)".*?/g,"$1"));
        }
                                    ...
                                    for (let k = 0; k < manifests.length; k++) {
                                        try {
                                            ...
                                            if(!resText2.description)
                                            {
                                                ...
                                                if(appLocation)resText2.description=getDescription(appLocation);
                                            }
