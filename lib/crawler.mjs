...
let manifests = ["https://gcode.com.au/apps/37790665-calculator/manifest.json","https://gcode.com.au/apps/37790665-boatcrew/manifest.json", "https://www.google.com/maps/preview/pwa/manifest?source=ttpwa&hl=en", "https://www.pinterest.com.au/manifest.json", "https://flipboard.com/manifest.webmanifest"];
for (let i = 0; i < oldFile.data.length; i++) {
    if (!manifests.includes(oldFile.data[i].root_uri)) manifests.push(oldFile.data[i].root_uri);
}
                                        ...
                                        (async () => {
                                            ...
                                            for (let k = 0; k < manifests.length; k++) {
                                                try {
                                                    ...
                                                    resText2.manifest_hits = manifesthits["man"+crypto.createHash('md5').update(manifests[k]).digest('hex')];
