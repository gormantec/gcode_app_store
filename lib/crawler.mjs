...
async function refreshGcodeApps() {
    ...
    let res2 = await fetch("https://github.com/gormantec/gcode/tree/master/apps");
    let htmlText = await res2.text();;
    let myArray = htmlText.match(/href\=\"\/gormantec\/gcode\/tree\/master\/apps\/([0-9]*?[a-zA-Z0-9-_\.]*?)\"/g);
    for (let i = 0; myArray != null && i < myArray.length; i++) {
        let aName = myArray[i].replace(/href\=\"\/gormantec\/gcode\/tree\/master\/apps\/([0-9]*?[a-zA-Z0-9-_\.]*?)\"/g, "$1");

        console.log(aName)
        if (!manifests.includes("https://gcode.com.au/apps/" + aName + "/manifest.json")) {
            manifests.unshift("https://gcode.com.au/apps/" + aName + "/manifest.json");
        }
    }
    manifests.unshift("https://store.gcode.com.au/manifest.json");
    manifests.unshift("https://gcode.com.au/manifest.webmanifest");
    ...
    for (let k = 0; k < manifests.length; k++) {
        try {
            ...
            if (fileMD5z[fileMD5] == null && resText2 != null && resText2.lang != "fa-IR" && resText2.lang != "fa" && !contain_persian_char(resText2.name)) {
                ...
                if (manifests[k].startsWith("https://gcode.com.au")) {
                    let APP_NAME = manifests[k].replace("/manifest.json", "");
                    APP_NAME = APP_NAME.replace("https://gcode.com.au/apps/", "");
                    const plistFile="https://gcode.com.au/apps/" + APP_NAME + "/ios/" + APP_NAME + ".plist";
                    try {
                        let res4 = await fetch(plistFile, { method: 'HEAD' });
                        if (res4.ok && res4.headers.get('content-type').indexOf("application") >= 0) {
                            console.log("Add iOS: " + plistFile);
                            resText2.ipa_location = "itms-services://?action=download-manifest&url=https://gcode.com.au/apps/" + APP_NAME + "/ios/" + APP_NAME + ".plist";
                        }
                    }
                    catch (e) {
                        console.log("e: " + e);
                    }
                    let apkLocation = resText2.root_uri.replace("/manifest.json", "/apk/"+short_name+".apk");
...
await seachGoogle("https://www.google.com/search?q=PWA+apps+list");
await seachGoogle("https://www.google.com/search?q=PWA");
