...
async function recurseCrawler(url, deep) {
    ...
    if (sw != "" && manFile != "") {
        ...
        if (!manifests.includes(manFile)) {
            ...
            manifesthits[crypto.createHash('md5').update(manFile).digest('hex')]=1;
            //console.log(manFile + ":: " + url);
        }
        else{
            manifesthits[crypto.createHash('md5').update(manFile).digest('hex')]=manifesthits[crypto.createHash('md5').update(manFile).digest('hex')]+1;
        }
    }
                                        ...
                                        (async () => {
                                            ...
                                            for (let k = 0; k < manifests.length; k++) {
                                                try {
                                                    ...
                                                    resText2.manifest_hits = manifesthits[crypto.createHash('md5').update(manifests[k]).digest('hex')];
